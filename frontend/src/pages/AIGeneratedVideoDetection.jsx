...
import { Video, Upload, AlertTriangle, CheckCircle, Film, BarChart2, Clock, Sparkles } from 'lucide-react'
import axios from 'axios'
import { useAuth } from '../context/AuthContext'
import { saveToHistory } from '../services/history'
import { aiGeneratedVideoAPI, commonAPI } from '../services/api'

const API_BASE = 'http://localhost:8000'

export default function AIGeneratedVideoDetection() {
  ...
  const analyze = async () => {
    ...
    setResult(null)
    try {
      const { data } = await aiGeneratedVideoAPI.analyzeVideo(uploadedUrl)
      setResult(data)

      if (currentUser) {
        await saveToHistory(currentUser.uid, 'AI Generated Video', data)
      }
    } catch (e) {
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="font-display text-2xl font-bold text-cyber-text">AI-Generated Video Detection</h1>
            <p className="text-cyber-muted text-sm">Detect AI-generated or synthetic videos using EfficientNetV2-S frame analysis</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div
            ...
            className={`relative border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all duration-200
              ${dragOver ? 'border-pink-400 bg-pink-500/10' : 'border-cyber-border hover:border-pink-500/50 hover:bg-pink-500/5'}`}
          >
          ...
          <button
            ...
            className="w-full py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-pink-500 to-violet-600 hover:opacity-90 disabled:opacity-40 transition-all duration-200 flex items-center justify-center gap-2"
          >
            {uploading ? (
              <>
        ...
        <AnimatePresence>
          {result && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              {/* Verdict */}
              <div className={`p-6 rounded-2xl border-2 ${isAI ? 'border-pink-500/50 bg-pink-500/10' : 'border-emerald-500/50 bg-emerald-500/10'}`}>
                <div className="flex items-center gap-4">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${isAI ? 'bg-pink-500/20' : 'bg-emerald-500/20'}`}>
                    {isAI ? <Sparkles className="w-8 h-8 text-pink-400" /> : <CheckCircle className="w-8 h-8 text-emerald-400" />}
                  </div>
                  <div>
                    <p className="text-cyber-muted text-xs font-mono uppercase tracking-wider mb-1">Verdict</p>
                    <h2 className={`text-2xl font-bold font-display ${isAI ? 'text-pink-400' : 'text-emerald-400'}`}>{result.verdict}</h2>
              ...
              <div className="p-5 rounded-xl bg-cyber-card border border-cyber-border">
                ...
                <div className="w-full h-3 bg-cyber-border rounded-full overflow-hidden">
                  <motion.div
                    ...
                    className={`h-full rounded-full ${isAI ? 'bg-gradient-to-r from-pink-500 to-violet-500' : 'bg-gradient-to-r from-emerald-500 to-cyan-500'}`}
                  />
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-xs text-cyber-muted">0%</span>
                  <span className={`text-xs font-bold ${isAI ? 'text-pink-400' : 'text-emerald-400'}`}>{result.confidence}%</span>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: 'AI Frames', value: result.ai_frames, icon: Sparkles, color: 'text-pink-400' },
              ...
              <div className="p-5 rounded-xl bg-cyber-card border border-cyber-border">
                ...
                <div className="flex rounded-lg overflow-hidden h-4">
                  <div style={{ width: `${(result.ai_frames / result.frames_analyzed) * 100}%` }} className="bg-pink-500 transition-all" />
                  <div style={{ width: `${(result.real_frames / result.frames_analyzed) * 100}%` }} className="bg-emerald-500 transition-all" />
                </div>
                <div className="flex gap-4 mt-2">
                  <span className="flex items-center gap-1 text-xs text-pink-400"><span className="w-2 h-2 rounded-full bg-pink-500 inline-block" />AI Generated</span>
