...
try:
    ...
    print("pillow-heif not installed, HEIC images may fail")
...
def analyze_ai_generated(files: List[UploadFile] = File(...)):
    if not ai_gen_model:
    ...
    for file in files:
        try:
            ...
            img_tensor = preprocess_ai_image(io.BytesIO(contents))
            with torch.no_grad():
...
def analyze_deepfake_image(files: List[UploadFile] = File(...)):
    start_time = time.time()
    results = []
    ...
    if not df_model:
        ...
        for file in files:
            try:
                ...
                print(f"Error processing {file.filename}: {e}")
                results.append({"filename": file.filename, "error": str(e), "is_deepfake": False, "confidence": 0})
            
    GLOBAL_STATS["total_scans"] += len(files)
    GLOBAL_STATS["threats_detected"] += sum(1 for r in results if r.get("is_deepfake", False))
    save_stats(GLOBAL_STATS)
