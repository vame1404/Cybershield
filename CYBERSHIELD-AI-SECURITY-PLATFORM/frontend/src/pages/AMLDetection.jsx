...
export default function AMLDetection() {
  ...
  const analyzeTransaction = async (e) => {
    ...
    const isSuspicious = amount > 10000 || 
                        transactionData.country?.toLowerCase().includes('high-risk') ||
                        Math.random() > 0.6
    
    const riskLevel = isSuspicious && amount > 50000 ? 'high' : (isSuspicious ? 'medium' : 'low')
    ...
    setResult({
      ...
      isSuspicious,
      riskLevel,
      riskScore: isSuspicious ? (60 + Math.random() * 35).toFixed(0) : (5 + Math.random() * 25).toFixed(0),
      anomalyFactors: isSuspicious ? {
        amountAnomaly: amount > 10000 ? 'Above threshold ($10,000)' : 'Within normal range',
        velocityCheck: Math.random() > 0.5 ? 'Unusual transaction frequency' : 'Normal velocity',
        geographicRisk: 'Cross-border transaction detected',
        behaviorPattern: 'Deviates from historical pattern',
        networkAnalysis: Math.random() > 0.5 ? 'Connected to flagged accounts' : 'No suspicious connections'
      } : {
        amountAnomaly: 'Within normal range',
        velocityCheck: 'Normal velocity',
        geographicRisk: 'Domestic transaction',
        behaviorPattern: 'Consistent with history',
        networkAnalysis: 'No suspicious connections'
      },
      ...
      ] : ['No action required', 'Transaction appears legitimate'],
